...
namespace JWMSY
{
    ...
    /// 原料标签打印界面
    /// </summary>
    public partial class WorkRmPrintLabel : Form
    {
        /// <summary>
        /// 进行记录跳转
        /// </summary>
        ...
        /// 原料标签打印构造函数
        /// </summary>
        public WorkRmPrintLabel(string lid)
        {
            _lid = lid;
            InitializeComponent();
        }
        ...
        {
            iT_RawMaterialTableAdapter.Connection.ConnectionString = BaseStructure.WmsCon;
            iT_RawMaterialTableAdapter.Fill(dataInventory.IT_RawMaterial, true);
        }
        ...
        {
            using (var brm = new Base_RawMaterial(true))
            {
        ...
        public void SetPanelVlaue(string lId, string llocation = "")
        {
            ...
            {
                case "btnLast":
                    sqlcmd.CommandText = "select top 1 * from Bar_RawMaterial order by AutoID desc";
                    break;
                case "btnFirst":
                    sqlcmd.CommandText = "select top 1 * from Bar_RawMaterial order by AutoID";
                    break;
                case "btnPre":
                    ...
                    sqlcmd.CommandText = "select top 1 * from Bar_RawMaterial where AutoID<@AutoID order by AutoID desc";
                ...
                case "btnNext":
                    ...
                    sqlcmd.CommandText = "select top 1 * from Bar_RawMaterial where AutoID>@AutoID order by AutoID";
                    sqlcmd.Parameters.AddWithValue("@AutoID", lId);
                    break;
                case "":
                    sqlcmd.CommandText = "select top 1 * from Bar_RawMaterial where AutoID=@AutoID";
                    sqlcmd.Parameters.AddWithValue("@AutoID", lId);
                    break;
            }
            ...
            using (var con = new SqlConnection(BaseStructure.WmsCon))
            {
                ...
                using (var dr = sqlcmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        ...
                        if (string.IsNullOrEmpty(dr["dMassDate"].ToString()))
                        {
                            dtpdMassDate.Checked = false;
                            dtpdMassDate.Value = DateTime.Now;
                        }
                        else
                        {
                            dtpdMassDate.Checked = true;
                            dtpdMassDate.Value = (DateTime)dr["dMassDate"];
                        }
                        if (string.IsNullOrEmpty(dr["dReceiveDate"].ToString()))
                        {
                            dReceiveDate.Checked = false;
                            dReceiveDate.Value = DateTime.Now;
                        }
                        else
                        {
                            dReceiveDate.Checked = true;
                            dReceiveDate.Value = (DateTime)dr["dReceiveDate"];
                        }
                    }
                    else
                    {
        ...
        private void biSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            using (var con = new SqlConnection(BaseStructure.WmsCon))
            {
                using (var cmd = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = con })
                {
                    ...
                    cmd.Parameters.AddWithValue("@dMassDate", dtpdMassDate.Value.Date);
                    cmd.Parameters.AddWithValue("@dReceiveDate", dReceiveDate.Value.Date);
                    con.Open();
        ...
        {
            var cBarCode = String.Format(@"RM*{0}*C*{1}*L*{2}", txtcInvCode.Text, lblTitleMain.lblcSerialNumber.Text,txtcLotNo.Text);
            var pList = new List<List<string>>
            {
                ...
                new List<string> {"dDate", dtpdDate.Value.ToString("yyyy-MM-dd")},
                ...
                new List<string> {"dReceiveDate",dReceiveDate.Value.Date.ToString("yyyy-MM-dd")},
        ...
        private void biEditPrinter_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
