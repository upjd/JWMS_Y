...
namespace JWMSY
{
    class WmsFunction
    {
        ...
        public static bool BoolCanOperate(string fCode, string fName)
        {
            //开始读取权限,先显示Group再显示Items
            const string sqlstr = "select * from view_RoleFunction where rCode=@rCode and fCode=@fCode+convert(nvarchar(10),id) and cRame=@fName ";
