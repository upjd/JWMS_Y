...
namespace JWMSY
{
    ...
    /// 半成品打印标签界面
    /// </summary>
    public partial class WorkSemiPrintLabel : Form
    {
        /// <summary>
        /// 进行记录跳转
        /// </summary>
        ...
        /// 半成品标签打印界面构造函数
        /// </summary>
        public WorkSemiPrintLabel(string lid)
        {
            _lid = lid;
            InitializeComponent();
        }
        ...
        private void SetDefaultValue(UltraGridRow uRow)
        {
            ...
            txtcDefaultSource.Text = uRow.Cells["cDefaultSource"].Value.ToString();
        ...
        private void WorkSemiPrintLabel_Load(object sender, EventArgs e)
        {
            GetRawMaterial();
            ...
            DllWorkPrintLabel.GetTemplet("半成品标签", ref _cCaption, ref _cPrinter, ref _cTempletFileName);
        ...
        {
            iT_SemiTableAdapter.Connection.ConnectionString = BaseStructure.WmsCon;
            iT_SemiTableAdapter.Fill(dataInventory.IT_Semi, true);
        }
        ...
        {
            using (var brm = new Base_Semi(true))
            {
        ...
        public void SetPanelVlaue(string lId, string llocation = "")
        {
            ...
            {
                case "btnLast":
                    sqlcmd.CommandText = "select top 1 * from Bar_Semi order by AutoID desc";
                    break;
                case "btnFirst":
                    sqlcmd.CommandText = "select top 1 * from Bar_Semi order by AutoID";
                    break;
                case "btnPre":
                    ...
                    sqlcmd.CommandText = "select top 1 * from Bar_Semi where AutoID<@AutoID order by AutoID desc";
                ...
                case "btnNext":
                    ...
                    sqlcmd.CommandText = "select top 1 * from Bar_Semi where AutoID>@AutoID order by AutoID";
                    sqlcmd.Parameters.AddWithValue("@AutoID", lId);
                    break;
                case "":
                    sqlcmd.CommandText = "select top 1 * from Bar_Semi where AutoID=@AutoID";
                    sqlcmd.Parameters.AddWithValue("@AutoID", lId);
                    break;
            }
            ...
            using (var con = new SqlConnection(BaseStructure.WmsCon))
            {
                ...
                using (var dr = sqlcmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        ...
                        txtcDefaultSource.Text = dr["cDefaultSource"].ToString();
        ...
        private void biSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            using (var con = new SqlConnection(BaseStructure.WmsCon))
            {
                using (var cmd = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = con })
                {
                    if (string.IsNullOrEmpty(lblTitleMain.lblAutoID.Text))
                    {
                        cmd.CommandText = "proc_Bar_SemiInsert";
                    ...
                    {
                        cmd.CommandText = "proc_Bar_SemiUpdate";
                    ...
                    cmd.Parameters.AddWithValue("@cDefaultSource", txtcDefaultSource.Text);
        ...
        private void PrinterDone()
        {
            ...
            var pList = new List<List<string>>
            {
                ...
                new List<string> {"cDefaultSource", txtcDefaultSource.Text},
                new List<string> {"cMassUnit", lblcMassUnit.Text},
                new List<string> {"dMassDate", dtpdMassDate.Value.Date.ToString("yyyy-MM-dd")},
        ...
        {
            using (var st = new Base_Templet(true, "半成品标签"))
            {
